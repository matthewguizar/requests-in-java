...
// CODE HERE
sayHelloButton.addEventListener('mouseover', (e) =>{
...
// CODE HERE
sayHelloButton.addEventListener('mouseout', (e) =>{
...
const ohMy = () => {
    ...
    .then((rez) =>{
        ohMy.textContent = rez.data;
        for (let i = 0; i < rez.data.length; i++){
            const p = document.createElement("p")
            p.textContent = rez.data[i]
            console.log(rez.data[i])
            
        };
        
    })
}
const ohMyBtn = document.querySelector('#animals-button').addEventListener('click', ohMy)
...
*/
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Smelly-BELLY')
    .then(rez =>{ 
      console.log(rez.data)
        const repeatText = document.querySelector('#repeat-text')
        repeatText.textContent = rez.data
        repeatText.style.display = 'block'
        
    })
}
    const repeatBtn = document.querySelector('#repeat-button') .addEventListener('click', repeatMyParam)
...
const test = () => {
    axios.get('http://localhost:3000/query-test?HELLO-MOTHER-HELLOFAT!!HER?interesting0-')
    .then (rez => {
        console.log(rez)
     } )
    }
    let querybtn = document.querySelector('#query-button').addEventListener('Click', test)
...
queryBtn.addEventListener('Click', test);
